...
.\backend\venv\Scripts\python.exe -m uvicorn backend.main:app --host 127.0.0.1 --port 8013
```
...
Frontend API traffic is configured through `REACT_APP_API_URL`. The committed local development default is `http://localhost:8013` via `frontend/.env.development`.

### Model Validation
```powershell
.\backend\venv\Scripts\python.exe backend\scripts\validate_models.py
```

The validation script loads both trained models, runs them against fixed sample images in `backend/test_samples/`, prints per-image predictions, and exits non-zero if model loading or inference fails.
