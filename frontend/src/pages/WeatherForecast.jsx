    ...
    const fetchForecast = async () => {
      try {
        ...
        if (response.data && response.data.status === 'success' && response.data.data.length > 0) {
          ...
          setIsLive(true);
          console.log('✅ Weather forecast loaded from API');
        }
      } catch (error) {
        console.warn('⚠️ Using fallback weather data:', error.message);
      }
    };
    fetchForecast();
  }, []);
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center md:items-start relative z-10">
            <div className="mb-4 md:mb-0">
              <div className="inline-flex items-center px-3 py-1 mb-3 rounded-full bg-smart-green text-white text-sm animate-bounce-slow">
                <span className="mr-1">●</span> {isLive ? 'Live Forecast' : 'Cached Data'}
              </div>
        ...
        <div className="mt-6 text-center text-gray-500 text-sm">
          <div className={`inline-block transform transition-all duration-1000 ${animateCloud ? 'rotate-1 scale-102' : 'rotate-0 scale-100'}`}>
            <p>Data updated: {new Date().toLocaleDateString('en-US', {month: 'long', day: 'numeric', year: 'numeric'})} | All times local</p>
            <p className="mt-1 text-xs">Powered by SmartCropX Weather</p>
          </div>
        </div>
      </div>
