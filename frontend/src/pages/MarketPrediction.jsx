...
const predictionData = [
  ...
  },
  {
    ...
    trend: "up",
    category: "vegetable",
    recommendation: "Steady price increase trend suggests holding inventory if storage conditions permit. Moderate upward trajectory indicates stable demand."
  },
  {
    crop: "tomato",
    unit: "Rs./Kg",
    ...
    trend: "stable",
    category: "vegetable",
    recommendation: "Focus on quality over quantity to secure premium pricing. Consider local direct-to-consumer sales for better margins."
  },
  {
    crop: "wheat",
    unit: "Rs./Kg",
    ...
    trend: "up",
    category: "grain",
    recommendation: "Sharp price increase expected in early April. Consider futures contracts to lock in higher prices."
  },
  {
    crop: "carrot",
    unit: "Rs./Kg",
    ...
    trend: "up",
    category: "vegetable",
    recommendation: "Strong upward trend indicates high demand. Consider staggered harvesting to benefit from peak prices."
  }
];
...
const MarketPrediction = () => {
  ...
  const [xaiLoading, setXaiLoading] = useState({});
  ...
  useEffect(() => {
    // Merge API data with static fallback metadata
    let dataToFilter = predictionData;
    
    if (apiData && apiData.length > 0) {
      // Merge: use API predictions/graph_url but fallback to static for category/recommendation
      const staticMap = {};
      predictionData.forEach(item => { staticMap[item.crop] = item; });
      
      dataToFilter = apiData.map(apiItem => {
        const staticItem = staticMap[apiItem.crop] || {};
        return {
          ...staticItem,
          ...apiItem,
          // Use API fields if present, otherwise keep static
          category: apiItem.category || staticItem.category || "other",
          recommendation: apiItem.recommendation || staticItem.recommendation || "Monitor price trends and plan accordingly.",
          trend: apiItem.trend || staticItem.trend || "stable",
          image: apiItem.graph_url || staticItem.image || "",
        };
      });
    }
  ...
  const fetchCropExplanation = async (crop) => {
    if (xaiData[crop]) return; // already fetched
    setXaiLoading(prev => ({ ...prev, [crop]: true }));
    try {
      const response = await axios.get(`${ENDPOINTS.EXPLAIN_PRICE}/${crop}`);
      setXaiData(prev => ({ ...prev, [crop]: response.data }));
    } catch (err) {
      setXaiData(prev => ({ ...prev, [crop]: { status: 'error', explanation: 'Could not load explanation.' } }));
    } finally {
      setXaiLoading(prev => ({ ...prev, [crop]: false }));
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
                          ...
                          <div className="md:col-span-2">
                            ...
                            <div className="bg-gray-800 rounded-lg p-4 mb-4">
                              <div className="flex items-center justify-between mb-3">
                                ...
                                {!xaiData[item.crop] && (
                                  <button
                                    onClick={(e) => { e.stopPropagation(); fetchCropExplanation(item.crop); }}
                                    disabled={xaiLoading[item.crop]}
                                    className="text-sm px-3 py-1 rounded bg-purple-600 hover:bg-purple-500 text-white transition"
                                  >
                                    {xaiLoading[item.crop] ? 'Loading...' : 'Explain'}
                                  </button>
                                )}
                              </div>

                              {xaiData[item.crop] && xaiData[item.crop].status === 'success' && (
                                <div className="animate-fadeIn">
                                  <p className="text-gray-300 text-sm mb-3">{xaiData[item.crop].explanation}</p>
                                  <span className="inline-block mb-3 text-xs px-2 py-1 rounded bg-purple-900 text-purple-200">
                                    Method: {xaiData[item.crop].method}
                                  </span>
                                  <div className="space-y-2">
                                    {xaiData[item.crop].features && xaiData[item.crop].features.map((f, fi) => (
                                      <div key={fi} className="bg-gray-900 rounded-lg p-3">
                                        <div className="flex items-center justify-between mb-1">
                                          <span className="text-sm font-medium text-white">{f.feature}</span>
                                          <span className={`text-xs font-medium px-2 py-0.5 rounded ${
                                            f.direction === 'positive' ? 'bg-green-900 text-green-300' :
                                            f.direction === 'negative' ? 'bg-red-900 text-red-300' :
                                            'bg-yellow-900 text-yellow-300'
                                          }`}>
                                            {f.direction === 'positive' ? '↑ Increases' : f.direction === 'negative' ? '↓ Decreases' : '↔ Mixed'} price
                                          </span>
                                        </div>
                                        <p className="text-gray-500 text-xs mb-1">{f.description}</p>
                                        <div className="w-full bg-gray-700 rounded-full h-2">
                                          <div
                                            className={`h-2 rounded-full ${
                                              f.direction === 'positive' ? 'bg-green-500' :
                                              f.direction === 'negative' ? 'bg-red-500' :
                                              'bg-yellow-500'
                                            }`}
                                            style={{ width: `${Math.min(f.impact * 400, 100)}%` }}
                                          />
                                        </div>
                                        <span className="text-gray-500 text-xs">Impact: {f.impact.toFixed(4)}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                              ...
                              {xaiData[item.crop] && xaiData[item.crop].status === 'error' && (
                                <p className="text-red-300 text-sm">{xaiData[item.crop].explanation || xaiData[item.crop].error}</p>
                              )}
                            </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-gray-400 text-sm">
            ...
            <br />Updated daily. Powered by SmartCropX.
          </p>
        </div>
      </div>
    </div>
