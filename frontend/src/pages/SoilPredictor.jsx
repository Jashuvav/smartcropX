...
function SoilPredictor() {
    ...
    try {
      ...
      if (res.data.error || res.data.message === "Error analyzing image") {
        setResult({
          ...
          soil_type: "Analysis Error",
          confidence: 0,
          why: "Uncertain soil prediction. Please upload a clearer soil image.",
          best_crops: []
        });
      } else {
        setResult(res.data);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-green-100 py-12 px-4 sm:px-6 pt-24">
              ...
              {result.why && (
                <div className={`mt-4 p-4 ${result.error ? 'bg-red-100 border border-red-200' : 'bg-white'} rounded-lg shadow-inner transition-all duration-300 hover:shadow-md`}>
                  <h4 className={`font-bold ${result.error ? 'text-red-700' : 'text-gray-700'}`}>
                    {result.error ? 'What happened:' : 'Why this soil?'}
                  </h4>
                  <p className={`mt-2 ${result.error ? 'text-red-600' : 'text-gray-600'}`}>{result.why}</p>
                </div>
              )}
                  ...
                  {result.best_crops && result.best_crops.length > 0 && (
                    <div className="p-4 bg-white rounded-lg shadow transition-all duration-300 hover:shadow-md">
                      ...
                      <ul className="mt-3 space-y-2">
                        {result.best_crops.map((crop, index) => (
                          <li 
                            key={crop} 
      ...
      <div className="text-center mt-6 text-gray-500 text-sm">
        © 2026 Soil Predictor AI | Using advanced ML for accurate soil analysis
      </div>
    </div>
  );
}
