...
function SoilPredictor() {
  ...
  const explainSoil = async () => {
    ...
    try {
      ...
      setXaiResult({ status: 'error', explanation: 'Could not generate explanation.' });
      setShowXai(true);
    } finally {
        ...
        <div className="p-8">
          ...
          {result && (
            <div className={`mt-8 border ${result.error ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'} rounded-xl p-6 transition-all duration-500 ${animateResult ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-10'}`}>
              ...
              {/* XAI: Explain Soil Prediction */}
              {!result.error && (
                <div className="mt-6">
                  <button
                    onClick={explainSoil}
                    disabled={isExplaining}
                    className="w-full py-3 px-4 rounded-lg bg-purple-600 hover:bg-purple-500 text-white font-medium transition-all duration-300 flex items-center justify-center"
                  >
                    {isExplaining ? (
                      ...
                      <><span className="mr-2">🔍</span> Why this soil type? (Explainable AI)</>
                    )}
                  </button>

                  {showXai && xaiResult && (
                    <div className="mt-4 border border-purple-200 bg-purple-50 rounded-xl p-5 animate-fadeIn">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-lg font-bold text-purple-700 flex items-center">
                          <span className="mr-2">🧠</span> Explainable AI Analysis
                        </h4>
                        <button onClick={() => setShowXai(false)} className="text-gray-400 hover:text-gray-600 text-sm">✕ Close</button>
                      </div>

                      {xaiResult.status === 'error' ? (
                        <p className="text-red-600 text-sm">{xaiResult.explanation}</p>
                      ) : (
                        <>
                          <p className="text-gray-600 text-sm mb-3">{xaiResult.explanation}</p>
                          <span className="inline-block mb-3 text-xs px-2 py-1 rounded bg-purple-200 text-purple-700">
                            Method: {xaiResult.method}
                          </span>

                          {/* Grad-CAM images */}
                          {(xaiResult.heatmap || xaiResult.overlay) && (
                            <div className="grid grid-cols-2 gap-3 mb-4">
                              {xaiResult.heatmap && (
                                <div>
                                  <p className="text-gray-500 text-xs mb-1">Activation Heatmap</p>
                                  <img src={xaiResult.heatmap} alt="Heatmap" className="w-full rounded-lg border border-purple-300"/>
                                </div>
                              )}
                              {xaiResult.overlay && (
                                <div>
                                  <p className="text-gray-500 text-xs mb-1">Overlay on Original</p>
                                  <img src={xaiResult.overlay} alt="Overlay" className="w-full rounded-lg border border-purple-300"/>
                                </div>
                              )}
                            </div>
                          )}
                          ...
                          {/* Feature explanation */}
                          {xaiResult.feature_explanation && xaiResult.feature_explanation.features && (
                            <div>
                              <h5 className="text-sm font-semibold text-purple-700 mb-2">Key Visual Features</h5>
                              <div className="space-y-2">
                                {xaiResult.feature_explanation.features.map((f, i) => (
                                  <div key={i} className="bg-white rounded-lg p-3 shadow-sm">
                                    <div className="flex items-center justify-between mb-1">
                                      <span className="text-sm font-medium text-gray-800">{f.feature}</span>
                                      <span className={`text-xs font-medium px-2 py-0.5 rounded ${
                                        f.direction === 'positive' ? 'bg-green-100 text-green-700' :
                                        f.direction === 'negative' ? 'bg-red-100 text-red-700' :
                                        'bg-yellow-100 text-yellow-700'
                                      }`}>
                                        {f.direction === 'positive' ? '↑ Supports' : f.direction === 'negative' ? '↓ Against' : '↔ Neutral'}
                                      </span>
                                    </div>
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-2">
                                      <div
                                        className={`h-2 rounded-full ${
                                          ...
                                          f.direction === 'negative' ? 'bg-red-400' : 'bg-yellow-400'
                                        }`}
                                        style={{ width: `${Math.min(f.impact * 300, 100)}%` }}
                                      />
                                    </div>
                                  </div>
                                ))}
                          ...
                          {xaiResult.regions && xaiResult.regions.length > 0 && (
                            <div className="mt-3">
                              <h5 className="text-sm font-semibold text-purple-700 mb-1">Top Activation Regions</h5>
                              <div className="space-y-1">
                                {xaiResult.regions.map((r, i) => (
                                  <div key={i} className="flex items-center text-sm text-gray-600">
                                    ...
                                    <span className="ml-2 text-xs">{(r.intensity * 100).toFixed(0)}%</span>
                                  </div>
                                ))}
                              </div>
                            </div>
